...
@property (nonatomic, strong) BaselineMethodUpdateView *dropdownMenu;/** 下拉菜单 */
...
-(void)checkCreateTeam{
    ...
    [OuterMinifyReflexive getWithUrl:Server_check_createteam params:nil isShow:NO success:^(id responseObject) {
...
- (NSArray *)getMenuModelsArray {    
    ...
    BaselineMethodUpdateModel *menuModel1 = [BaselineMethodUpdateModel ff_DropDownMenuModelWithMenuItemTitle:activity menuItemIconName:@"ic_create_chat" menuBlock:^{
        if (_creatTeam.integerValue != 0) {
            [PromptOutlineFloraInstantiateReceiver showMessage:LangKey(@"please_contact_your_administrator")];
        }else{
    ...
    //菜单模型1
    @weakify(self);
    BaselineMethodUpdateModel *menuModel3 = [BaselineMethodUpdateModel ff_DropDownMenuModelWithMenuItemTitle:LangKey(@"fragment_contact_new_scan") menuItemIconName:@"ic_scan" menuBlock:^{
        @strongify(self);
        [self requestAuthorizationForVideo];
    }];
...
-(void)newGroupSyncRequest:(NSString *)groupName teamID:(NSString *)teamId{
    ...
    dict[@"name"] = groupName;
    dict[@"id"] = teamId;
    [OuterMinifyReflexive getWithUrl:Server_team_create params:dict isShow:NO success:^(id responseObject) {
...
{
    if(!_noticeView){
        _noticeView = [[MixerFrostSlice alloc]initWithFrame:CGRectMake(0, SCREEN_STATUS_HEIGHT+140, SCREEN_WIDTH, SCREEN_SAFE_HEIGHT-140)];
