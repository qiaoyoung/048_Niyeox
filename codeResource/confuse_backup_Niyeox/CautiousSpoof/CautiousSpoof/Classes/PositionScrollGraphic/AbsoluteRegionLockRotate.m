...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [PromptOutlineFloraInstantiateReceiver showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
...
- (PullBeforeMonochrome *)selectIndicatorView{
    ...
    _selectIndicatorView = [[PullBeforeMonochrome alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
...
- (PatternAggregatorSunnyListener *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[PatternAggregatorSunnyListener alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
