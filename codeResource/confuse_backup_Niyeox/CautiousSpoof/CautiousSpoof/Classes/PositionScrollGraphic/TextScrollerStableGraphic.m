...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [PrioritizeTender passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [PrioritizeTender setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
