...
// __CLOSE_PRINT__
//
//  SurfPragmaticFileScaleWisdom.m
// ViewAngleFind
//
...
//: @interface SurfPragmaticFileScaleWisdom ()
@interface SurfPragmaticFileScaleWisdom ()

//: @property (nonatomic) CGFloat badgeLeftPadding; 
@property (nonatomic) CGFloat current;//数字左部到红圈的距离

//: @property (nonatomic) CGFloat whiteCircleWidth; 
@property (nonatomic) CGFloat cap;//最外层白圈的宽度

//: @property (nonatomic) CGFloat badgeTopPadding; 
@property (nonatomic) CGFloat smart;//数字顶部到红圈的距离

//: @property (strong) UIColor *badgeTextColor;
@property (strong) UIColor *envelopeWhisperRecover;

//: @property (nonatomic) UIFont *badgeTextFont;
@property (nonatomic) UIFont *secure;

//: @property (strong) UIColor *badgeBackgroundColor;
@property (strong) UIColor *fixed;

//: @end
@end

//: @implementation SurfPragmaticFileScaleWisdom
@implementation SurfPragmaticFileScaleWisdom

//: #pragma mark - Private
#pragma mark - Private
//: - (void)drawWithContent:(CGRect)rect context:(CGContextRef)context{
- (void)disturbing:(CGRect)rect player:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
    //: CGRect bkgFrame = CGRectInset(self.bounds, self.whiteCircleWidth, self.whiteCircleWidth);
    CGRect bkgFrame = CGRectInset(self.bounds, self.cap, self.cap);
    ...
    CGRect badgeSize = CGRectInset(self.bounds, self.cap + self.current, self.cap + self.smart);
    //: if ([self badgeBackgroundColor]) {
    if ([self fixed]) {//外白色描边
        //: CGContextSetFillColorWithColor(context, [[UIColor whiteColor] CGColor]);
        CGContextSetFillColorWithColor(context, [[UIColor whiteColor] CGColor]);
        //: if ([self badgeValue].integerValue > 9) {
        if ([self arrayTechnique].integerValue > 9) {
        ...
        CGContextSetFillColorWithColor(context, [[self fixed] CGColor]);
        //: if ([self badgeValue].integerValue > 9) {
        if ([self arrayTechnique].integerValue > 9) {
    ...
    CGContextSetFillColorWithColor(context, [[self envelopeWhisperRecover] CGColor]);
    ...
    NSDictionary *badgeTextAttributes = @{
                                          ...
                                          NSFontAttributeName: [self secure],
                                          //: NSForegroundColorAttributeName: [self badgeTextColor],
                                          NSForegroundColorAttributeName: [self envelopeWhisperRecover],
                                          //: NSParagraphStyleAttributeName: badgeTextStyle,
                                          NSParagraphStyleAttributeName: badgeTextStyle,
                                          //: };
                                          };
    //: [[self badgeValue] drawInRect:CGRectMake(self.whiteCircleWidth + self.badgeLeftPadding,
    [[self arrayTechnique] drawInRect:CGRectMake(self.cap + self.current,
...
+ (instancetype)magicPraise:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    }
    //: SurfPragmaticFileScaleWisdom *instance = [[SurfPragmaticFileScaleWisdom alloc] init];
    SurfPragmaticFileScaleWisdom *instance = [[SurfPragmaticFileScaleWisdom alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance moreValuable:badgeValue];
    //: instance.badgeValue = badgeValue;
    instance.arrayTechnique = badgeValue;
...
- (void)setArrayTechnique:(NSString *)badgeValue {
    //: _badgeValue = badgeValue;
    _arrayTechnique = badgeValue;
    //: if (_badgeValue.integerValue > 9) {
    if (_arrayTechnique.integerValue > 9) {
    ...
    self.frame = [self moreValuable:badgeValue];
...
- (void)pull:(CGRect)rect hard:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
...
- (CGSize)fullPending:(NSString *)badgeValue{
    ...
    CGSize size = [badgeValue sizeWithAttributes:@{NSFontAttributeName:self.secure}];
...
- (CGRect)moreValuable:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self fullPending:badgeValue];
    ...
    CGRect badgeFrame = CGRectMake(self.frame.origin.x, self.frame.origin.y, badgeSize.width + self.current * 2 + self.cap * 2, badgeSize.height + self.smart * 2 + self.cap * 2);//8=2*2（红圈-文字）+2*2（白圈-红圈）
    //: return badgeFrame;
    return badgeFrame;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self arrayTechnique] length]) {
        //: [self drawWithContent:rect context:context];
        [self disturbing:rect player:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self pull:rect hard:context];
    }
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _fixed = [UIColor redColor];
//        _badgeBackgroundColor = RGB_COLOR_String(@"#612CF9");
        //: _badgeTextColor = [UIColor whiteColor];
        _envelopeWhisperRecover = [UIColor whiteColor];
        //: _badgeTextFont = [UIFont boldSystemFontOfSize:12];
        _secure = [UIFont boldSystemFontOfSize:12];
