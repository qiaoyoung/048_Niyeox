...
// __CLOSE_PRINT__
//
//  GraciousSummarizeLabelDiameterWavy.m
//  NIM
//
...
//: @implementation GraciousSummarizeLabelDiameterWavy
@implementation GraciousSummarizeLabelDiameterWavy

//: + (unsigned long long)fileSize:(NSString *)filepath
+ (unsigned long long)close:(NSString *)filepath
{
...
+ (void)piece:(NSString *)filepath performHighlight:(void(^)(NSString *MD5))completion;
{
    ...
    dispatch_block_t block = ^(){
        ...
        NSString *md5 = [self windCapacity:filepath];
        //: if(completion) {
        if(completion) {
            //: completion(md5);
            completion(md5);
...
+ (NSString *)windCapacity:(NSString *)filepath
{
    //: NSString *md5 = nil;
    NSString *md5 = nil;
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filepath])
    {
        ...
        unsigned long long fileSize = [self close:filepath];
        //: if(fileSize > 200 * 1024 * 1024) {
        if(fileSize > 200 * 1024 * 1024) {
            //: md5 = [self bigFileMD5:filepath];
            md5 = [self anyHost:filepath];
        //: }else {
        }else {
...
+ (NSString *)anyHost:(NSString *)filePath
{
