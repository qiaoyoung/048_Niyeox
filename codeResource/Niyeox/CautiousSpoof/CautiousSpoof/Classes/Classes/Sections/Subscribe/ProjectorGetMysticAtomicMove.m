//
//  ProjectorGetMysticAtomicMove.m
//  NIM
//
...
@interface ProjectorGetMysticAtomicMove()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@implementation ProjectorGetMysticAtomicMove

+ (instancetype)sharedManager
{
    static ProjectorGetMysticAtomicMove *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[ProjectorGetMysticAtomicMove alloc] init];
    });
    return instance;
...
- (void)publishOnlineState
{
    ...
    event.value = TopicVigorousFlagshipValueOnlineExt;
    event.sendToOnlineUsersOnly = NO;  //必须要让后登录的用户也能拿到    
    NSDictionary *ext = @{@"net_state" : @([ScalabilityCircuitPingPrefetchWhiten currentDevice].currentNetworkType),
                          @"online_state" : @(WaitAmendOrbitStateNormal), //移动端永远在线
                          };
