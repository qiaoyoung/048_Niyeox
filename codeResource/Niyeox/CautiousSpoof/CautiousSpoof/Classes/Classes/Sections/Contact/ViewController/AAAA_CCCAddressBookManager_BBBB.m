//
//  AAAA_CCCAddressBookManager_BBBB.m
//  sohunews
//
...
@interface AAAA_CCCAddressBookManager_BBBB () <CNContactViewControllerDelegate, CNContactPickerDelegate>
...
@implementation AAAA_CCCAddressBookManager_BBBB

dSINGLETON_FOR_CLASS(AAAA_CCCAddressBookManager_BBBB);
...
- (void)accessContactsWithMobileComplection:(void (^)(BOOL succeed, NSArray <NSDictionary *> *mobileContacts))completcion {
    [self accessContactsComplection:^(BOOL succeed, NSArray<AAAA_CCCContactPerson_BBBB *> *contacts) {
        ...
        if (contacts.count > 0) {
            for (AAAA_CCCContactPerson_BBBB *person in contacts) {
                for (AAAA_NIMContactPhone_BBBB *model in person.phones) {
...
- (void)accessContactsComplection:(void (^)(BOOL, NSArray<AAAA_CCCContactPerson_BBBB *> *))completcion {
...
- (void)accessSectionContactsComplection:(void (^)(BOOL, NSArray<AAAA_LJSectionPerson_BBBB *> *, NSArray<NSString *> *))completcion {
...
- (void)_asynAccessContactStoreWithSort:(BOOL)isSort completcion:(void (^)(NSArray *, NSArray *))completcion {
    ...
    dispatch_async(_queue, ^{
        ...
        [weakSelf.contactStore enumerateContactsWithFetchRequest:request error:nil usingBlock:^(CNContact * _Nonnull contact, BOOL * _Nonnull stop) {
            ...
            AAAA_CCCContactPerson_BBBB *person = [[AAAA_CCCContactPerson_BBBB alloc] initWithCNContact:contact];
...
- (void)_sortNameWithDatas:(NSArray *)datas completcion:(void (^)(NSArray *, NSArray *))completcion {
    ...
    for (AAAA_CCCContactPerson_BBBB *person in datas)
    {
        // 拼音首字母
    ...
    [keys enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        AAAA_LJSectionPerson_BBBB *person = [AAAA_LJSectionPerson_BBBB new];
        person.key = key;
        
        // 组内按照拼音排序
        NSArray *personsArr = [dict[key] sortedArrayUsingComparator:^NSComparisonResult(AAAA_CCCContactPerson_BBBB *person1, AAAA_CCCContactPerson_BBBB *person2) {
