...
@class AAAA_FFDropDownMenu_BBBBView;
...
#define FFDefaultCell @"AAAA_FFDropDownMenu_BBBBCell"
#define FFDefaultMenuScaleType AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight
 ...
 *  菜单展现的动画类型
 */
typedef NS_ENUM(NSInteger, AAAA_FFDropDownMenu_BBBBViewAnimateType) {
    /**
     *  scale based the point of upper right corner.
     *  以右上角为基点进行伸缩
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight = 0, //default
    /**
     *  scale based the point of upper left corner.
     *  以左上角为基点进行伸缩
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopLeft,
    /**
     *  scale based the middle point
     *  以中点为基点进行伸缩
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedMiddle,
    /**
     *  fade in fade out
     *  淡入淡出效果
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_FadeInFadeOut,
    /**
     *  animate like RollerShutter
     *  卷帘效果
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_RollerShutter,
    /**
     *  fall from top
     *  从上往下落下
     */
    AAAA_FFDropDownMenu_BBBBViewAnimateType_FallFromTop,
};


@protocol AAAA_FFDropDownMenu_BBBBViewDelegate <NSObject>
...
- (void)AAAA_FFDropDownMenu_BBBBView:(AAAA_FFDropDownMenu_BBBBView *)menuView WillAppearMenuCell:(AAAA_FFDropDownMenu_BBBBBasedCell *)menuCell index:(NSInteger)index;

- (void)AAAA_FFDropDownMenu_BBBBViewWillAppear;

- (void)AAAA_FFDropDownMenu_BBBBViewWDidAppear;

- (void)AAAA_FFDropDownMenu_BBBBViewWillDisappear;

- (void)AAAA_FFDropDownMenu_BBBBViewWDidDisappear;
 ...
 *  下拉菜单
 */
@interface AAAA_FFDropDownMenu_BBBBView : UIView
 ...
 *
 *
 *  @param menuModelsArray     下拉菜单模型数组,数组中存放着(AAAA_FFDropDownMenu_BBBBModel模型对象)
 ...
 *
 *  @param menuModelsArray     drop-down menu models array. Array storage AAAA_FFDropDownMenu_BBBBModel instances
...
///   >>步骤:
///   1、 [AAAA_FFDropDownMenu_BBBBView alloc] init]
///   2、 对需要赋值的属性进行赋值
...
/** 1、下拉菜单模型数组(数组里面存放 AAAA_FFDropDownMenu_BBBBBasedModel的子类的对象) 
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down menu models array(Array storage AAAA_FFDropDownMenu_BBBBModel instances or the AAAA_FFDropDownMenu_BBBBModel's subclass instances )
 */
@property (nonatomic, strong) NSArray *menuModelsArray;

/** 2、cell的类名, 必须是AAAA_FFDropDownMenu_BBBBBasedCell的子类
 *               若用框架自带的cell,直接传@"AAAA_FFDropDownMenu_BBBBCell"
 *               若使用自定义的cell,就传自定义cell的类名,若cell是xib,则传@"类名.xib"
 *                     举例如下:
 *                            ----->非xibcell   举例:@"YourCustomMenuCell"
 *                            ----->xib的cell   举例:@"YourCustomMenuCell.xib"
 *
 *               若不设置，默认为 @"AAAA_FFDropDownMenu_BBBBCell"
 *
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     drop-down nemu cell's class name. Must be a subclass of AAAA_FFDropDownMenu_BBBBBasedCell
 *              If want to use this framework's drop-down nemu cell, direct pass @"AAAA_FFDropDownMenu_BBBBCell"
...
/** 15、菜单的弹出的动画效果类型(若不设置，默认为AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight)
 *     -----------------------------------------------------------
 *     English description:
 *     -----------------------------------------------------------
 *     show drop-down menu animate type. (if not assign, default value is AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight)
 */
@property (nonatomic, assign) AAAA_FFDropDownMenu_BBBBViewAnimateType menuAnimateType;
 ...
 *
 */
@property (nonatomic, weak) id<AAAA_FFDropDownMenu_BBBBViewDelegate> delegate;
