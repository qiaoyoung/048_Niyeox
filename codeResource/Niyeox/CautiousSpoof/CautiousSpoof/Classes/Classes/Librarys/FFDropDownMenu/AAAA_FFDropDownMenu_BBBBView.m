...
@interface AAAA_FFDropDownMenu_BBBBView ()<UITableViewDataSource, UITableViewDelegate>

/**tableView*/
@property (nonatomic, weak)  UITableView *tableView;

@property (nonatomic, strong) AAAA_FFDropDownMenu_BBBBTriangleView *triangleView;
...
@implementation AAAA_FFDropDownMenu_BBBBView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.cellClassName = @"AAAA_FFDropDownMenu_BBBBCell";
        self.menuWidth = 150;
        ...
        self.menuAnimateType = AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight;
...
+ (instancetype)ff_DefaultStyleDropDownMenuWithMenuModelsArray:(NSArray *)menuModelsArray menuWidth:(CGFloat)menuWidth eachItemHeight:(CGFloat)eachItemHeight menuRightMargin:(CGFloat)menuRightMargin triangleRightMargin:(CGFloat)triangleRightMargin {
    ...
    AAAA_FFDropDownMenu_BBBBView *menuView = [AAAA_FFDropDownMenu_BBBBView new];
...
- (UITableView *)tableView {
    if (_tableView == nil) {
        ...
        switch (self.menuAnimateType) {
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopRight: //右上角
                tableView.layer.anchorPoint = CGPointMake(1, 0);
                break;
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedTopLeft: //左上角
                tableView.layer.anchorPoint = CGPointMake(0, 0);
                break;
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_ScaleBasedMiddle: //中间
                break;
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_FadeInFadeOut: //淡入淡出效果
                break;
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_RollerShutter: //卷帘效果
                tableView.layer.anchorPoint = CGPointMake(0.5, 1);
                break;
            case AAAA_FFDropDownMenu_BBBBViewAnimateType_FallFromTop:
            break;
        ...
        if ([self.cellClassName hasSuffix:@".xib"]) { //xib名称
            ...
            if (![NSClassFromString(className) isSubclassOfClass:[AAAA_FFDropDownMenu_BBBBBasedCell class]]) {
                FFLog(@"%@这个类不是AAAA_FFDropDownMenu_BBBBBasedCell的子类,请继承这个类",className);
                return _tableView;
            }
            ...
            if (![NSClassFromString(self.cellClassName) isSubclassOfClass:[AAAA_FFDropDownMenu_BBBBBasedCell class]]) {
                FFLog(@"%@这个类不是AAAA_FFDropDownMenu_BBBBBasedCell的子类,请继承这个类",self.cellClassName);
                return _tableView;
            }
...
- (AAAA_FFDropDownMenu_BBBBTriangleView *)triangleView {
    if (_triangleView == nil) {
        AAAA_FFDropDownMenu_BBBBTriangleView *triangleView = [[AAAA_FFDropDownMenu_BBBBTriangleView alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    AAAA_FFDropDownMenu_BBBBBasedCell *cell = [tableView dequeueReusableCellWithIdentifier:CellID];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.backgroundColor = [UIColor clearColor];
    AAAA_FFDropDownMenu_BBBBBasedModel *menuModel = self.menuModelsArray[indexPath.row];
    
    //如果用框架中默认的菜单样式，则隐藏最后一个菜单的下划线
    
    if ([cell isMemberOfClass:[AAAA_FFDropDownMenu_BBBBCell class]]) {
        
        AAAA_FFDropDownMenu_BBBBCell *tempCell = (AAAA_FFDropDownMenu_BBBBCell *)cell;
    ...
    if ([self.delegate respondsToSelector:@selector(AAAA_FFDropDownMenu_BBBBView:WillAppearMenuCell:index:)]) {
        [self.delegate AAAA_FFDropDownMenu_BBBBView:self WillAppearMenuCell:cell index:indexPath.row];
    }
    return cell;
}
...
- (void)dismissMenuWithAnimation:(BOOL)animation {
    ...
    if (animation == YES) {
        ...
        if (self.menuAnimateType == AAAA_FFDropDownMenu_BBBBViewAnimateType_FadeInFadeOut) {
        ...
        else if (self.menuAnimateType == AAAA_FFDropDownMenu_BBBBViewAnimateType_RollerShutter) {
        ...
        else if (self.menuAnimateType == AAAA_FFDropDownMenu_BBBBViewAnimateType_FallFromTop) {
...
- (void)menuWillShow {
    ...
    if ([self.delegate respondsToSelector:@selector(AAAA_FFDropDownMenu_BBBBViewWillAppear)]) {
        [self.delegate AAAA_FFDropDownMenu_BBBBViewWillAppear];
    }
}

- (void)menuDidShow {

    if ([self.delegate respondsToSelector:@selector(AAAA_FFDropDownMenu_BBBBViewWDidAppear)]) {
        [self.delegate AAAA_FFDropDownMenu_BBBBViewWDidAppear];
    }
}

- (void)menuWillDisappear {
    
    if ([self.delegate respondsToSelector:@selector(AAAA_FFDropDownMenu_BBBBViewWillDisappear)]) {
        [self.delegate AAAA_FFDropDownMenu_BBBBViewWillDisappear];
    }
}

- (void)menuDidDisappear {
    if ([self.delegate respondsToSelector:@selector(AAAA_FFDropDownMenu_BBBBViewWDidDisappear)]) {
        [self.delegate AAAA_FFDropDownMenu_BBBBViewWDidDisappear];
    }
...
- (void)setMenuAnimateType:(AAAA_FFDropDownMenu_BBBBViewAnimateType)menuAnimateType { //15
