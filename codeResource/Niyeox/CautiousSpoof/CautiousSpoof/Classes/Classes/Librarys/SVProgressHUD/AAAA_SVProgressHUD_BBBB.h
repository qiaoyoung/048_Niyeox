...
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBDidReceiveTouchEventNotification;
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBDidTouchDownInsideNotification;
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBWillDisappearNotification;
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBDidDisappearNotification;
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBWillAppearNotification;
extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBDidAppearNotification;

extern NSString * _Nonnull const AAAA_SVProgressHUD_BBBBStatusUserInfoKey;

/// Represents the appearance style of the HUD.
typedef NS_ENUM(NSInteger, AAAA_SVProgressHUD_BBBBStyle) {
    /// White HUD with black text. HUD background will be blurred.
    AAAA_SVProgressHUD_BBBBStyleLight NS_SWIFT_NAME(light),
    
    /// Black HUD with white text. HUD background will be blurred.
    AAAA_SVProgressHUD_BBBBStyleDark NS_SWIFT_NAME(dark),
    
    /// Uses the fore- and background color properties.
    AAAA_SVProgressHUD_BBBBStyleCustom NS_SWIFT_NAME(custom),
    
    /// Automatically switch between light or dark mode appearance.
    AAAA_SVProgressHUD_BBBBStyleAutomatic NS_SWIFT_NAME(automatic)
};

/// Represents the type of mask to be applied when the HUD is displayed.
typedef NS_ENUM(NSUInteger, AAAA_SVProgressHUD_BBBBMaskType) {
    /// Allow user interactions while HUD is displayed.
    AAAA_SVProgressHUD_BBBBMaskTypeNone NS_SWIFT_NAME(none) = 1,
    
    /// Don't allow user interactions with background objects.
    AAAA_SVProgressHUD_BBBBMaskTypeClear NS_SWIFT_NAME(clear),
    
    /// Don't allow user interactions and dim the UI behind the HUD (as in iOS 7+).
    AAAA_SVProgressHUD_BBBBMaskTypeBlack NS_SWIFT_NAME(black),
    
    /// Don't allow user interactions and dim the UI with an UIAlertView-like background gradient (as in iOS 6).
    AAAA_SVProgressHUD_BBBBMaskTypeGradient NS_SWIFT_NAME(gradient),
    
    /// Don't allow user interactions and dim the UI behind the HUD with a custom color.
    AAAA_SVProgressHUD_BBBBMaskTypeCustom NS_SWIFT_NAME(custom)
};

/// Represents the animation type of the HUD when it's shown or hidden.
typedef NS_ENUM(NSUInteger, AAAA_SVProgressHUD_BBBBAnimationType) {
    /// Custom flat animation (indefinite animated ring).
    AAAA_SVProgressHUD_BBBBAnimationTypeFlat NS_SWIFT_NAME(flat),
    
    /// iOS native UIActivityIndicatorView.
    AAAA_SVProgressHUD_BBBBAnimationTypeNative NS_SWIFT_NAME(native)
};

typedef void (^AAAA_SVProgressHUD_BBBBShowCompletion)(void);
typedef void (^AAAA_SVProgressHUD_BBBBDismissCompletion)(void);

@interface AAAA_SVProgressHUD_BBBB : UIView

#pragma mark - Customization

/// Represents the default style for the HUD.
/// @discussion Default: AAAA_SVProgressHUD_BBBBStyleAutomatic.
@property (assign, nonatomic) AAAA_SVProgressHUD_BBBBStyle defaultStyle UI_APPEARANCE_SELECTOR;

/// Represents the type of mask applied when the HUD is displayed.
/// @discussion Default: AAAA_SVProgressHUD_BBBBMaskTypeNone.
@property (assign, nonatomic) AAAA_SVProgressHUD_BBBBMaskType defaultMaskType UI_APPEARANCE_SELECTOR;

/// Defines the animation type used when the HUD is displayed.
/// @discussion Default: AAAA_SVProgressHUD_BBBBAnimationTypeFlat.
@property (assign, nonatomic) AAAA_SVProgressHUD_BBBBAnimationType defaultAnimationType UI_APPEARANCE_SELECTOR;
...
/// @discussion These implicitly set the HUD's style to `AAAA_SVProgressHUD_BBBBStyleCustom`.
...
+ (void)dismissWithCompletion:(nullable AAAA_SVProgressHUD_BBBBDismissCompletion)completion;
...
/// @param delay The time in seconds after which the HUD should be dismissed.
/// @param completion A block that gets executed after the HUD is dismissed.
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(nullable AAAA_SVProgressHUD_BBBBDismissCompletion)completion;
