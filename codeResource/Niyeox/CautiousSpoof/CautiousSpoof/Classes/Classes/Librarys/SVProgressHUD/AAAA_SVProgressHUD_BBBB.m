...
#error AAAA_SVProgressHUD_BBBB is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
NSString * const AAAA_SVProgressHUD_BBBBDidReceiveTouchEventNotification = @"AAAA_SVProgressHUD_BBBBDidReceiveTouchEventNotification";
NSString * const AAAA_SVProgressHUD_BBBBDidTouchDownInsideNotification = @"AAAA_SVProgressHUD_BBBBDidTouchDownInsideNotification";
NSString * const AAAA_SVProgressHUD_BBBBWillDisappearNotification = @"AAAA_SVProgressHUD_BBBBWillDisappearNotification";
NSString * const AAAA_SVProgressHUD_BBBBDidDisappearNotification = @"AAAA_SVProgressHUD_BBBBDidDisappearNotification";
NSString * const AAAA_SVProgressHUD_BBBBWillAppearNotification = @"AAAA_SVProgressHUD_BBBBWillAppearNotification";
NSString * const AAAA_SVProgressHUD_BBBBDidAppearNotification = @"AAAA_SVProgressHUD_BBBBDidAppearNotification";

NSString * const AAAA_SVProgressHUD_BBBBStatusUserInfoKey = @"AAAA_SVProgressHUD_BBBBStatusUserInfoKey";

static const CGFloat AAAA_SVProgressHUD_BBBBParallaxDepthPoints = 10.0f;
static const CGFloat AAAA_SVProgressHUD_BBBBUndefinedProgress = -1;
static const CGFloat AAAA_SVProgressHUD_BBBBDefaultAnimationDuration = 0.15f;
static const CGFloat AAAA_SVProgressHUD_BBBBVerticalSpacing = 12.0f;
static const CGFloat AAAA_SVProgressHUD_BBBBHorizontalSpacing = 12.0f;
static const CGFloat AAAA_SVProgressHUD_BBBBLabelSpacing = 8.0f;


@interface AAAA_SVProgressHUD_BBBB ()
...
@property (nonatomic, strong) AAAA_SVRadialGradientLayer_BBBB *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) AAAA_SVProgressAnimatedView_BBBB *ringView;
@property (nonatomic, strong) AAAA_SVProgressAnimatedView_BBBB *backgroundRingView;
...
@implementation AAAA_SVProgressHUD_BBBB {
    BOOL _isInitializing;
}

+ (AAAA_SVProgressHUD_BBBB*)sharedView {
    static dispatch_once_t once;
    
    static AAAA_SVProgressHUD_BBBB *sharedView;
#if !defined(SV_APP_EXTENSIONS)
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[AAAA_SVProgressHUD_BBBB mainWindow].bounds]; });
#else
...
//#else
//     NSBundle *bundle = [NSBundle bundleForClass:[AAAA_SVProgressHUD_BBBB class]];
//#endif
//     NSURL *url = [bundle URLForResource:@"AAAA_SVProgressHUD_BBBB" withExtension:@"bundle"];
...
+ (void)setDefaultStyle:(AAAA_SVProgressHUD_BBBBStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(AAAA_SVProgressHUD_BBBBMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(AAAA_SVProgressHUD_BBBBAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:AAAA_SVProgressHUD_BBBBStyleCustom];
}
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:AAAA_SVProgressHUD_BBBBUndefinedProgress status:status];
}
...
+ (void)dismissWithCompletion:(AAAA_SVProgressHUD_BBBBDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(AAAA_SVProgressHUD_BBBBDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = AAAA_SVProgressHUD_BBBBMaskTypeNone;
        _defaultStyle = AAAA_SVProgressHUD_BBBBStyleAutomatic;
        _defaultAnimationType = AAAA_SVProgressHUD_BBBBAnimationTypeFlat;
...
//        NSBundle *imageBundle = [AAAA_SVProgressHUD_BBBB imageBundle];
        ...
        _fadeInAnimationDuration = AAAA_SVProgressHUD_BBBBDefaultAnimationDuration;
        _fadeOutAnimationDuration = AAAA_SVProgressHUD_BBBBDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"AAAA_SVProgressHUD_BBBB";
...
- (void)updateHUDFrame {
    ...
    hudWidth = AAAA_SVProgressHUD_BBBBHorizontalSpacing + MAX(labelWidth, contentWidth) + AAAA_SVProgressHUD_BBBBHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = AAAA_SVProgressHUD_BBBBVerticalSpacing + labelHeight + contentHeight + AAAA_SVProgressHUD_BBBBVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += AAAA_SVProgressHUD_BBBBLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(AAAA_SVProgressHUD_BBBBVerticalSpacing, (self.minimumSize.height - contentHeight - AAAA_SVProgressHUD_BBBBLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != AAAA_SVProgressHUD_BBBBUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + AAAA_SVProgressHUD_BBBBLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-AAAA_SVProgressHUD_BBBBParallaxDepthPoints);
    effectX.maximumRelativeValue = @(AAAA_SVProgressHUD_BBBBParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-AAAA_SVProgressHUD_BBBBParallaxDepthPoints);
    effectY.maximumRelativeValue = @(AAAA_SVProgressHUD_BBBBParallaxDepthPoints);
...
#else
            // If AAAA_SVProgressHUD_BBBB is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{AAAA_SVProgressHUD_BBBBStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    self.frame =  [AAAA_SVProgressHUD_BBBB mainWindow].bounds;
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak AAAA_SVProgressHUD_BBBB *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong AAAA_SVProgressHUD_BBBB *strongSelf = weakSelf;
        if(strongSelf){
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = AAAA_SVProgressHUD_BBBBUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != AAAA_SVProgressHUD_BBBBMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(AAAA_SVProgressHUD_BBBBDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:AAAA_SVProgressHUD_BBBBDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
                    
                    // Tell the rootViewController to update the StatusBar appearance
#if !defined(SV_APP_EXTENSIONS) && TARGET_OS_IOS
                    UIViewController *rootController = [AAAA_SVProgressHUD_BBBB mainWindow].rootViewController;
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == AAAA_SVProgressHUD_BBBBAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[AAAA_SVIndefiniteAnimatedView_BBBB class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[AAAA_SVIndefiniteAnimatedView_BBBB alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        AAAA_SVIndefiniteAnimatedView_BBBB *indefiniteAnimatedView = (AAAA_SVIndefiniteAnimatedView_BBBB*)_indefiniteAnimatedView;
...
- (AAAA_SVProgressAnimatedView_BBBB*)ringView {
    if(!_ringView) {
        _ringView = [[AAAA_SVProgressAnimatedView_BBBB alloc] initWithFrame:CGRectZero];
    }
...
- (AAAA_SVProgressAnimatedView_BBBB*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[AAAA_SVProgressAnimatedView_BBBB alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    AAAA_SVProgressHUD_BBBBStyle style = [self defaultStyleResolvingAutomatic];
    
    if(style == AAAA_SVProgressHUD_BBBBStyleLight) {
        return [UIColor blackColor];
    } else if(style == AAAA_SVProgressHUD_BBBBStyleDark) {
...
- (UIControl*)controlView {
    ...
    // Update frame
#if !defined(SV_APP_EXTENSIONS)
    _controlView.frame = [AAAA_SVProgressHUD_BBBB mainWindow].bounds;
#else
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == AAAA_SVProgressHUD_BBBBMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [AAAA_SVRadialGradientLayer_BBBB layer];
        }
        ...
        if(self.defaultMaskType == AAAA_SVProgressHUD_BBBBMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == AAAA_SVProgressHUD_BBBBMaskTypeCustom){
...
- (AAAA_SVProgressHUD_BBBBStyle) defaultStyleResolvingAutomatic {
    if(self.defaultStyle == AAAA_SVProgressHUD_BBBBStyleAutomatic) {
        return self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark ? AAAA_SVProgressHUD_BBBBStyleDark : AAAA_SVProgressHUD_BBBBStyleLight;
    }
...
- (void)fadeInEffects {
    if(self.defaultStyle != AAAA_SVProgressHUD_BBBBStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle;
#if TARGET_OS_IOS
        if (@available(iOS 13.0, *)) {
            blurEffectStyle = [self defaultStyleResolvingAutomatic] == AAAA_SVProgressHUD_BBBBStyleLight ? UIBlurEffectStyleSystemMaterial : UIBlurEffectStyleSystemMaterialDark;
        } else {
            blurEffectStyle = [self defaultStyleResolvingAutomatic] == AAAA_SVProgressHUD_BBBBStyleLight ? UIBlurEffectStyleLight : UIBlurEffectStyleDark;
        }
#else
        blurEffectStyle = [self defaultStyleResolvingAutomatic] == AAAA_SVProgressHUD_BBBBStyleLight ? UIBlurEffectStyleLight : UIBlurEffectStyleDark;
#endif
...
- (void)setDefaultAnimationType:(AAAA_SVProgressHUD_BBBBAnimationType)animationType {
