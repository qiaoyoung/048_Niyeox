...
@interface AAAA_SGScanView_BBBB ()
@property (nonatomic, strong) AAAA_SGScanView_BBBBConfigure *configure;
...
@implementation AAAA_SGScanView_BBBB
...
+ (instancetype)scanViewWithFrame:(CGRect)frame configure:(AAAA_SGScanView_BBBBConfigure *)configure {
    return [[AAAA_SGScanView_BBBB alloc] initWithFrame:frame configure:configure];
}
...
- (void)leftTop:(CGFloat)borderX borderY:(CGFloat)borderY cornerLength:(CGFloat)cornerLength insideExcess:(CGFloat) insideExcess outsideExcess:(CGFloat)outsideExcess {
    ...
    if (self.configure.cornerLocation == AAAA_SGScanView_BBBBLoactionInside) {
    ...
    } else if (self.configure.cornerLocation == AAAA_SGScanView_BBBBLoactionOutside) {
...
- (void)startScanning {
    ...
    if (self.link == nil) {
        self.link = [CADisplayLink displayLinkWithTarget:[AAAA_SGWeakProxy_BBBB weakProxyWithTarget:self] selector:@selector(updateUI)];
