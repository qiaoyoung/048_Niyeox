...
- (NSData *)can:(NSString *)key name:(NSString *)vector {
...
void runContainer(const char *key, int keylen, char* output, const char* input, int len)
{
    // 设置密钥
    //: char key_[256];
    char key_[256];
...
- (NSData *)motion:(NSString *)key {
    ...
    runContainer(pwd, (int)key.length, outdata, self.bytes, (int)self.length);
...
- (NSData *)extended:(NSString *)key improvedFreshLikely:(NSString *)vector {
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    //: assert(key.length == 32);
    assert(key.length == 32);
    //: char keyPtr[kCCKeySizeAES256 + 1];
...
- (NSData *)pastPositionAnalyze:(NSString *)key {
     //: const char *pwd = [key UTF8String];
     const char *pwd = [key UTF8String];
