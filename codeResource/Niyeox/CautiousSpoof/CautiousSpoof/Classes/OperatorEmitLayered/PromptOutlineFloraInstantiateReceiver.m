...
#error PromptOutlineFloraInstantiateReceiver is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif
...
+ (PromptOutlineFloraInstantiateReceiver*)sharedView {
    ...
    static PromptOutlineFloraInstantiateReceiver *sharedView;
     ...
     dispatch_once(&once, ^{ sharedView = [[self alloc] initWithFrame:[PromptOutlineFloraInstantiateReceiver mainWindow].bounds]; });
#else
...
- (void)updateHUDFrame {
    ...
    hudWidth = PromptOutlineFloraInstantiateReceiverHorizontalSpacing + MAX(labelWidth, contentWidth) + PromptOutlineFloraInstantiateReceiverHorizontalSpacing;
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(PromptOutlineFloraInstantiateReceiverVerticalSpacing, (self.minimumSize.height - contentHeight - PromptOutlineFloraInstantiateReceiverLabelSpacing - labelHeight) / 2.0f);
...
#else
            // If PromptOutlineFloraInstantiateReceiver is used inside an app extension add it to the given view
