...
typedef NS_OPTIONS(NSUInteger, AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB) {
    AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Up,
    AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Down
};

@interface AAAA_FFFBubbleMenuView_BBBB()
...
@property (nonatomic, assign)AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB directionPriority;
...
@implementation AAAA_FFFBubbleMenuView_BBBB

+ (instancetype)shareMenuView {
    static AAAA_FFFBubbleMenuView_BBBB *menu = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (menu == nil) {
            menu = [[AAAA_FFFBubbleMenuView_BBBB alloc] init];
        }
    });
  return menu;
}
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _directionPriority = AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Up;
...
- (void)showViewWithButtonModels:(NSArray *)array
                 cursorStartRect:(CGRect)cursorStartRect selectionTextRectInWindow:(CGRect)rect selectBlock:(void(^)(AAAA_FFFMediaItem_BBBB *item))block praiseBlock:(void(^)(NSInteger tag))praiseblock {
    ...
    //默认的方向是优先方向
    AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB direction = _directionPriority;
//
...
//        //优先级Up的话，先判断在上面的情况。
        ...
        if (rect.origin.y - _bubbleContentMargin - _bubbleBottomMargin - praiseheight > _bubbleHeight) {
            ...
            direction = AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Up;
            ...
            direction = AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Down;
...
//    }
    ...
    bgViewRect.origin.y = direction == AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Down ? _arrowHeight : praiseheight;
...
//    }
    ...
    if (direction == AAAA_FFFBubbleMenuViewDirectionPriorityType_BBBB_Up) {
...
- (UIView *)drawButtonsWithArray:(NSArray *)array {
    ...
    for (int i = 0; i < array.count; i ++) {
        ...
        AAAA_FFFBubbleButtonModel_BBBB *model = array[i];
...
- (void)onButtonTouched:(UIButton *)button {
    if (_myNewContentArray.count > button.tag - 100) {
        AAAA_FFFBubbleButtonModel_BBBB *model = (AAAA_FFFBubbleButtonModel_BBBB *)_myNewContentArray[button.tag - 100];
...
- (BOOL)ifButtonsInfoChanged:(NSArray *)newArray {
    ...
    for (int i = 0; i < newArray.count; i ++) {
        BOOL finded = false;
        NSInteger newId = ((AAAA_FFFBubbleButtonModel_BBBB *)_oldContentArray[i]).buttonId;
        for (int j = 0; j < _oldContentArray.count; j ++) {
            NSInteger oldId = ((AAAA_FFFBubbleButtonModel_BBBB *)_oldContentArray[j]).buttonId;
            if (newId == oldId) {
                finded = true;
                continue;
            }
