...
@interface AAAA_FFFMessageModel_BBBB()
...
@implementation AAAA_FFFMessageModel_BBBB
...
{
    if (![object isKindOfClass:[AAAA_FFFMessageModel_BBBB class]])
    {
        return NO;
    }
    else
    {
        AAAA_FFFMessageModel_BBBB *model = object;
           ...
           completion:(void(^)(NSMapTable *))completion
{
                                                      ...
                                                      completion:^(NSError * _Nullable error, NSMapTable<NSNumber *,NIMQuickComment *> * _Nullable result)
    {
        if (completion)
        {
            if (result.count > 0)
            {
                _emoticonsContainerSize = [AAAA_FFFKitQuickCommentUtil_BBBB containerSizeWithComments:result];
            }
            completion(result);
        }
    }];
