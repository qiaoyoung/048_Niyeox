//
//  AAAA_FFFTeamCardSelectedViewController_BBBB.m
// AAAA_MyUserKit_BBBB
//
...
@interface AAAA_FFFTeamCardSelectedViewController_BBBB ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong) NSMutableArray <id <AAAA_FFFKitSelectCardData_BBBB>> *datas;
...
@implementation AAAA_FFFTeamCardSelectedViewController_BBBB

+ (instancetype)instanceWithTitle:(NSString *)title
                            items:(NSMutableArray <id <AAAA_FFFKitSelectCardData_BBBB>> *)items
                           result:(NIMSelectedCompletion)result {
    AAAA_FFFTeamCardSelectedViewController_BBBB *vc = [[AAAA_FFFTeamCardSelectedViewController_BBBB alloc] initWithItems:items];
    vc.titleString = title ?: @"";
    vc.resultHandle = result;
    return vc;
}

- (instancetype)initWithItems:(NSMutableArray <id <AAAA_FFFKitSelectCardData_BBBB>> *)items {
    if (self = [super init]) {
        _datas = items;
        _selectedIndex = -1;
        __weak typeof(self) weakSelf = self;
        [items enumerateObjectsUsingBlock:^(id<AAAA_FFFKitSelectCardData_BBBB>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if (obj.selected) {
...
- (void)onDone{
    if (_oriSelectedIndex != _selectedIndex) {
        id <AAAA_FFFKitSelectCardData_BBBB> bodyData = _datas[_selectedIndex];
        if (_resultHandle) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    id <AAAA_FFFKitSelectCardData_BBBB> bodyData = _datas[indexPath.section];
    
//    AAAA_FFFTeamCartSetTableViewCell_BBBB *cell = [AAAA_FFFTeamCartSetTableViewCell_BBBB cellWithTableView:tableView];
    NSString *identifier = [NSString stringWithFormat:@"ZMONTeamCartSet%ld",(long)indexPath.row];
    AAAA_FFFTeamCartSetTableViewCell_BBBB *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell)
    {
        cell = [[AAAA_FFFTeamCartSetTableViewCell_BBBB alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [_datas enumerateObjectsUsingBlock:^(id<AAAA_FFFKitSelectCardData_BBBB>  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
