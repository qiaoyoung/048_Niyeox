...
@interface AAAA_FFFSessionViewController_BBBB ()<NIMMediaManagerDelegate,AAAA_NIMInputDelegate_BBBB,AAAA_FFFTeamCardViewControllerDelegate_BBBB,NIMConversationManagerDelegate,AAAA_CustomUISeletePhotosDelegate_BBBB,UIGestureRecognizerDelegate,NIMUserManagerDelegate,NIMTeamManagerDelegate>
...
//    [self.view makeToast:serverExtensionString duration:2.0f position:CSToastPositionCenter];
    ...
    [AAAA_MyUserDefaults_BBBB standardUserDefaults].noScreen = noScreen;
}
-(void)NIMSDKConnect{
...
- (void)saveValueInDefault:(NSString *)key{
    [[AAAA_MyUserDefaults_BBBB standardUserDefaults] setAccid:@""];
}
    ...
    if(team.type == NIMTeamTypeAdvanced){
            vc = [[AAAA_FFFAdvancedTeamCardViewController_BBBB alloc] initWithTeam:team
                                                                 session:self.session
                                                                  option:option];
            vc.delegate = self;
        ((AAAA_FFFAdvancedTeamCardViewController_BBBB *)vc).canAddFriend = self.canAddFriend;
    }
...
-(AAAA_FFFInputView_BBBB *)sessionInputView{
    if (!_sessionInputView) {
        if ([self shouldShowInputView])
        {
            _sessionInputView = [[AAAA_FFFInputView_BBBB alloc] initWithFrame:CGRectMake(0, 0, self.view.device_width,0) config:self.sessionConfig];
            ...
            [_sessionInputView refreshStatus:AAAA_NIMInputStatus_BBBBText];
