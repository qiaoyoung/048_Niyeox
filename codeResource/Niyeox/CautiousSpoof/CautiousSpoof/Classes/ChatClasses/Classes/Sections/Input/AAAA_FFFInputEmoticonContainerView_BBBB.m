//
//  AAAA_FFFInputEmoticonContainerView_BBBB.m
// AAAA_MyUserKit_BBBB
//
//  Created by chris.
...
@interface AAAA_FFFInputEmoticonContainerView_BBBB()<AAAA_NIMEmoticonButtonTouchDelegate_BBBB,AAAA_NIMInputEmoticon_BBBBTabDelegate>

@property (nonatomic,strong) NSMutableArray *pageData;

@property (nonatomic,strong) AAAA_FFFInputEmoticonButton_BBBB* deleteIcon;

@end


@implementation AAAA_FFFInputEmoticonContainerView_BBBB
...
{
//    _emoticonPageView                  = [[AAAA_FFFPageView_BBBB alloc] initWithFrame:self.bounds];
...
//
//    _deleteIcon = [[AAAA_FFFInputEmoticonButton_BBBB alloc] init];
...
{
    AAAA_NIMInputEmoticonCatalog_BBBB *defaultCatalog = [self loadDefaultCatalog];
...
//    NSArray *catalogs = defaultCatalog? [@[defaultCatalog] arrayByAddingObjectsFromArray:charlets] : charlets;
        AAAA_NIMInputEmoticonCatalog_BBBB *gifCatalog = [self loadGifCatalog];
...
- (void)layoutEmoticonPageView
{
    ...
    if(self.currentCatalogData.emoticons.count>0){
        ...
        {
            AAAA_NIMInputEmoticon_BBBB *data = [self.currentCatalogData.emoticons objectAtIndex:index];
            
            AAAA_FFFInputEmoticonButton_BBBB *button = [AAAA_FFFInputEmoticonButton_BBBB iconButtonWithData:data catalogID:self.currentCatalogData.catalogID delegate:self];
            //计算表情位置
...
- (NSInteger)sumPages
{
    ...
    [self.totalCatalogData enumerateObjectsUsingBlock:^(AAAA_NIMInputEmoticonCatalog_BBBB* data, NSUInteger idx, BOOL *stop) {
...
- (UIView*)emojPageView:(AAAA_FFFPageView_BBBB*)pageView inEmoticonCatalog:(AAAA_NIMInputEmoticonCatalog_BBBB *)emoticon page:(NSInteger)page
{
    ...
    {
        AAAA_NIMInputEmoticon_BBBB *data = [emoticon.emoticons objectAtIndex:index];
        
        AAAA_FFFInputEmoticonButton_BBBB *button = [AAAA_FFFInputEmoticonButton_BBBB iconButtonWithData:data catalogID:emoticon.catalogID delegate:self];
        //计算表情位置
...
- (void)addDeleteEmotButtonToView:(UIView *)view
                ...
                inEmoticonCatalog:(AAAA_NIMInputEmoticonCatalog_BBBB *)emoticon
{
    AAAA_FFFInputEmoticonButton_BBBB* deleteIcon = [[AAAA_FFFInputEmoticonButton_BBBB alloc] init];
...
{
    NSInteger page  = 0;
    AAAA_NIMInputEmoticonCatalog_BBBB *emoticon;
...
- (AAAA_NIMInputEmoticonCatalog_BBBB*)loadDefaultCatalog
{
    AAAA_NIMInputEmoticonCatalog_BBBB *emoticonCatalog = [[AAAA_FFFInputEmoticonManager_BBBB sharedManager] emoticonCatalog:NEEKIT_EmojiCatalog];
    if (emoticonCatalog) {
        AAAA_NIMInputEmoticonLayout_BBBB *layout = [[AAAA_NIMInputEmoticonLayout_BBBB alloc] initEmojiLayout:self.device_width];
...
- (AAAA_NIMInputEmoticonCatalog_BBBB*)loadGifCatalog
{
    AAAA_NIMInputEmoticonCatalog_BBBB *emoticonCatalog = [[AAAA_FFFInputEmoticonManager_BBBB sharedManager] emoticonCatalog:NEEKIT_EmojiGifCatalog];
    if (emoticonCatalog) {
        AAAA_NIMInputEmoticonLayout_BBBB *layout = [[AAAA_NIMInputEmoticonLayout_BBBB alloc] initEmojiLayout:self.device_width];
        emoticonCatalog.layout = layout;
        emoticonCatalog.pagesCount = [self numberOfPagesWithEmoticon:emoticonCatalog];
...
- (NSArray *)loadChartlet{
    ...
    if ([self.config respondsToSelector:@selector(charlets)])
    {
        ...
        for (AAAA_NIMInputEmoticonCatalog_BBBB *item in chatlets) {
            AAAA_NIMInputEmoticonLayout_BBBB *layout = [[AAAA_NIMInputEmoticonLayout_BBBB alloc] initCharletLayout:self.device_width];
            item.layout = layout;
...
//找到某组表情的起始位置
- (NSInteger)pageIndexWithEmoticon:(AAAA_NIMInputEmoticonCatalog_BBBB *)emoticonCatalog{
    NSInteger pageIndex = 0;
    for (AAAA_NIMInputEmoticonCatalog_BBBB *emoticon in self.totalCatalogData) {
...
- (NSInteger)pageIndexWithTotalIndex:(NSInteger)index{
    AAAA_NIMInputEmoticonCatalog_BBBB *catelog = [self emoticonWithIndex:index];
    NSInteger begin = [self pageIndexWithEmoticon:catelog];
    return index - begin;
}

- (AAAA_NIMInputEmoticonCatalog_BBBB *)emoticonWithIndex:(NSInteger)index {
    NSInteger page  = 0;
    AAAA_NIMInputEmoticonCatalog_BBBB *emoticon;
    for (emoticon in self.totalCatalogData) {
        NSInteger newPage = page + emoticon.pagesCount;
...
- (NSInteger)numberOfPagesWithEmoticon:(AAAA_NIMInputEmoticonCatalog_BBBB *)emoticonCatalog
{
...
- (void)pageViewScrollEnd: (AAAA_FFFPageView_BBBB *)pageView
    ...
    AAAA_NIMInputEmoticonCatalog_BBBB *emticon = [self emoticonWithIndex:index];
...
- (void)setCurrentCatalogData:(AAAA_NIMInputEmoticonCatalog_BBBB *)currentCatalogData{
...
- (NSArray *)allEmoticons{
    ...
    for (AAAA_NIMInputEmoticonCatalog_BBBB *catalog in self.totalCatalogData) {
...
#pragma mark - Get
- (AAAA_FFFInputEmoticonTabView_BBBB *)tabView
{
    if (!_tabView) {
        _tabView = [[AAAA_FFFInputEmoticonTabView_BBBB alloc] initWithFrame:CGRectMake(0, 0, self.device_width, 0)];
