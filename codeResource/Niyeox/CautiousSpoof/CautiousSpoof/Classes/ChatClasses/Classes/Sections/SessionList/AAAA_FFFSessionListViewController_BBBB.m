//
//  AAAA_FFFSessionListViewController_BBBB.m
// AAAA_MyUserKit_BBBB
//
//  Created by NetEase.
...
@interface AAAA_FFFSessionListViewController_BBBB ()

//@property (nonatomic,strong)  UIImageView *navBarHairlineImageView;

@end

@implementation AAAA_FFFSessionListViewController_BBBB
...
- (void)tapGestureRecognizer:(id)sender {
    AAAA_MyWebViewController_BBBB *vc = [[AAAA_MyWebViewController_BBBB alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    AAAA_FFFSessionListCell_BBBB *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (!cell) {
        cell = [[AAAA_FFFSessionListCell_BBBB alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
...
- (void)onSelectedRecent:(NIMRecentSession *)recentSession atIndexPath:(NSIndexPath *)indexPath{
    AAAA_FFFSessionViewController_BBBB *vc = [[AAAA_FFFSessionViewController_BBBB alloc] initWithSession:recentSession.session];
...
- (NSString *)nameForRecentSession:(NIMRecentSession *)recent {
    if (recent.session.sessionType == NIMSessionTypeP2P) {
        return [AAAA_FFFKitUtil_BBBB showNick:recent.session.sessionId inSession:recent.session];
...
- (NSString *)timestampDescriptionForRecentSession:(NIMRecentSession *)recent{
    if (recent.lastMessage) {
        return [AAAA_FFFKitUtil_BBBB showTime:recent.lastMessage.timestamp showDetail:NO];
    }
    // 服务端时间戳以毫秒为单位,需要转化
    NSTimeInterval timeSecond = recent.updateTime / 1000.0;
    return [AAAA_FFFKitUtil_BBBB showTime:timeSecond showDetail:NO];
}
...
- (NSString *)messageContent:(NIMMessage*)lastMessage{
    NSString *text = [AAAA_FFFMessageUtil_BBBB messageContent:lastMessage];
    ...
    else
    {
        ...
        text = [AAAA_FFFMessageUtil_BBBB messageContent:msg];
        
        NSString *from = msg.from;
        NSString *nickName = [AAAA_FFFKitUtil_BBBB showNick:from inSession:msg.session];
