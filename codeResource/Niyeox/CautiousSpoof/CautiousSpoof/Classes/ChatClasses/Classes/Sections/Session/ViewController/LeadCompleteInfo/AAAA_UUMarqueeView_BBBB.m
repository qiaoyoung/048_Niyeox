...
@interface AAAA_UUMarqueeView_BBBB () <AAAA_UUMarqueeViewTouchResponder_BBBB>

@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, assign) NSInteger visibleItemCount;
@property (nonatomic, strong) NSMutableArray<AAAA_UUMarqueeItemView_BBBB*> *items;
@property (nonatomic, assign) int firstItemIndex;
@property (nonatomic, assign) int dataIndex;
@property (nonatomic, strong) NSTimer *scrollTimer;
@property (nonatomic, strong) AAAA_UUMarqueeViewTouchReceiver_BBBB *touchReceiver;
...
@implementation AAAA_UUMarqueeView_BBBB
...
- (void)resetAll {
    ...
    if (_direction == AAAA_UUMarqueeViewDirection_BBBBLeftward) {
    ...
    for (int i = 0; i < _visibleItemCount + 2; i++) {
        AAAA_UUMarqueeItemView_BBBB *itemView = [[AAAA_UUMarqueeItemView_BBBB alloc] init];
...
- (void)createItemView:(AAAA_UUMarqueeItemView_BBBB*)itemView {
...
- (void)updateItemView:(AAAA_UUMarqueeItemView_BBBB*)itemView atIndex:(NSInteger)index {
    if (index < 0) {
        [itemView clear];
    }
...
- (void)resetTouchReceiver {
    if (_touchEnabled) {
        if (!_touchReceiver) {
            self.touchReceiver = [[AAAA_UUMarqueeViewTouchReceiver_BBBB alloc] init];
...
- (void)touchesEndedAtPoint:(CGPoint)point {
    for (AAAA_UUMarqueeItemView_BBBB *itemView in _items) {
...
@implementation AAAA_UUMarqueeViewTouchReceiver_BBBB
...
@implementation AAAA_UUMarqueeItemView_BBBB
