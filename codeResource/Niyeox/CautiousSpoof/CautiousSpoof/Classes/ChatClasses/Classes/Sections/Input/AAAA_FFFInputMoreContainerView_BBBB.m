...
@interface AAAA_FFFInputMoreContainerView_BBBB() <AAAA_FFFPageViewDataSource_BBBB,AAAA_FFFPageViewDelegate_BBBB,AAAA_CustomMediaPickerViewDelegate_BBBB>
{
...
@property (nonatomic, strong) AAAA_FFFPageView_BBBB *pageView;

@end

@implementation AAAA_FFFInputMoreContainerView_BBBB

- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
//        _pageView = [[AAAA_FFFPageView_BBBB alloc] initWithFrame:CGRectZero];
...
- (void)setup {
    ...
    // 创建相册选择器视图
    _albumPickerView = [[AAAA_CustomAlbumPickerView_BBBB alloc] initWithFrame:CGRectMake(20, 10, SCREEN_WIDTH-40, 380)];
...
- (void)genMediaButtons
{
    ...
    if (!self.config)
    {
        items = [AAAA_MyUserKit_BBBB sharedKit].config.defaultMediaItems;
    }
...
- (UIView*)mediaPageView:(AAAA_FFFPageView_BBBB*)pageView beginItem:(NSInteger)begin endItem:(NSInteger)end
{
...
- (UIView*)oneLineMediaInPageView:(AAAA_FFFPageView_BBBB *)pageView
...
- (void)onTouchButton:(id)sender
{
    ...
    AAAA_FFFMediaItem_BBBB *item = _mediaItems[index];
