...
@interface AAAA_FFFKitProgressHUD_BBBB()

@property (nonatomic, strong) CAShapeLayer *indefiniteAnimatedLayer;

@end

@implementation AAAA_FFFKitProgressHUD_BBBB

+ (instancetype)sharedView
{
    static AAAA_FFFKitProgressHUD_BBBB *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[AAAA_FFFKitProgressHUD_BBBB alloc] initWithFrame:CGRectMake(0, 0, 84, 84)];
    });
    return instance;
}
...
+ (void)show
{
    [[AAAA_FFFKitProgressHUD_BBBB sharedView] showInView:[UIApplication sharedApplication].windows.firstObject];
}

+ (void)dismiss
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[AAAA_FFFKitProgressHUD_BBBB sharedView] removeFromSuperview];
        [[AAAA_FFFKitProgressHUD_BBBB sharedView].indefiniteAnimatedLayer removeFromSuperlayer];
    });
}
...
//        NSString *bundleName = [[AAAA_MyUserKit_BBBB sharedKit] resourceBundleName];
//        NSURL *bundleURL = [[NSBundle bundleForClass:[AAAA_FFFKitProgressHUD_BBBB class]] URLForResource:bundleName withExtension:nil];
//        
