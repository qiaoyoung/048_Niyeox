...
@interface AAAA_FFFSessionAudioContentView_BBBB()<NIMMediaManagerDelegate>
...
@implementation AAAA_FFFSessionAudioContentView_BBBB
...
- (void)refreshBackground:(AAAA_FFFMessageModel_BBBB *)data
{
...
- (void)layoutSubviews{
    ...
    switch (self.layoutStyle) {
        case AAAA_FFFSessionMessageContentView_BBBBLayoutLeft: {
...
//            self.voiceImageView.left = contentInsets.left * 2;
        ...
        case AAAA_FFFSessionMessageContentView_BBBBLayoutRight: {
//            _voiceImageView.transform = CGAffineTransformMakeRotation(M_PI);
        ...
        case AAAA_FFFSessionMessageContentView_BBBBLayoutAuto:
        default:
        {
...
- (BOOL)isPlaying
{
    BOOL play = [AAAA_FFFKitAudioCenter_BBBB instance].currentPlayingMessage == self.model.message; //对比是否是同一条消息，严格同一条，不能是相同ID，防止进了会话又进云端消息界面，导致同一个ID的云消息也在动画
    return play;
}
