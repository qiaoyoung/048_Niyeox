//
//  AAAA_FFFContactSelectViewController_BBBB.m
// AAAA_MyUserKit_BBBB
//
...
@interface AAAA_FFFContactSelectViewController_BBBB ()<UITableViewDataSource, UITableViewDelegate, AAAA_FFFContactPickedViewDelegate_BBBB>{
    NSMutableArray *_selectecContacts;
}
@property (strong, nonatomic) UITableView *tableView;
@property(nonatomic, strong) AAAA_FFFSetGroupNameView_BBBB *setGroupnameView;
@property(nonatomic, strong) UIView *headerview;
@property(nonatomic, strong) UIButton *selectedBtn;
@property(nonatomic, strong) UITextField *textfield;
@property(nonatomic, strong) UIImage *headPortrait;

@property (strong, nonatomic) AAAA_FFFContactSelectTabView_BBBB *selectIndicatorView;
...
@implementation AAAA_FFFContactSelectViewController_BBBB
...
- (void)setConfig:(id<AAAA_FFFContactSelectConfig_BBBB>)config{
    _config = config;
...
- (IBAction)onDoneBtnClick:(id)sender {
    ...
    if (_selectecContacts.count == 0) {
        [AAAA_SVProgressHUD_BBBB showMessage:LangKey(@"main_activity_choose_least_one")];
        return;
    }
        ...
        self.setGroupnameView.speiceBackBlock = ^(NSString *groupName){
            @strongify(self)
                if (_selectecContacts.count) {
                    ...
                    AAAA_FFFGroupAvatarViewController_BBBB *vc = [[AAAA_FFFGroupAvatarViewController_BBBB alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    id<AAAA_NIMGroupMemberProtocol_BBBB> contactItem = arr[indexPath.row];
    
    AAAA_FFFContactDataCell_BBBB *cell = [tableView dequeueReusableCellWithIdentifier:@"SelectContactCellID"];
    if (cell == nil) {
        cell = [[AAAA_FFFContactDataCell_BBBB alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"SelectContactCellID"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *title = self.sectionTitles[indexPath.section];
    NSMutableArray *arr = [self.contentDic valueForKey:title];
    id<AAAA_NIMGroupMemberProtocol_BBBB> member = arr[indexPath.row];

    NSString *memberId = [(id<AAAA_NIMGroupMemberProtocol_BBBB>)member memberId];
    AAAA_FFFContactDataCell_BBBB *cell = (AAAA_FFFContactDataCell_BBBB *)[tableView cellForRowAtIndexPath:indexPath];
    AAAA_FFFKitInfo_BBBB *info;
...
- (AAAA_FFFContactSelectTabView_BBBB *)selectIndicatorView{
    if (_selectIndicatorView) {
        return _selectIndicatorView;
    }

    _selectIndicatorView = [[AAAA_FFFContactSelectTabView_BBBB alloc] initWithFrame:CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, 64)];
    _selectIndicatorView.doneButton.hidden = YES;
    return _selectIndicatorView;
}

- (AAAA_FFFSetGroupNameView_BBBB *)setGroupnameView{
    if(!_setGroupnameView){
        _setGroupnameView = [[AAAA_FFFSetGroupNameView_BBBB alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)];
    }
