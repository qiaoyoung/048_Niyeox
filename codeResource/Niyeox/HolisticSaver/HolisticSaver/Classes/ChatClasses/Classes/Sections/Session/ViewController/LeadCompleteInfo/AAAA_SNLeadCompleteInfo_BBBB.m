//
//  AAAA_SNLeadCompleteInfo_BBBB.m
//  sohunews
//
...
@interface AAAA_SNLeadCompleteInfo_BBBB () <AAAA_UUMarqueeViewDelegate_BBBB>
...
@implementation AAAA_SNLeadCompleteInfo_BBBB
...
+ (instancetype)showTipViewForCompletingUserInfolWithDelay:(float)delay
    ...
    AAAA_SNLeadCompleteInfo_BBBB *tipView = [[AAAA_SNLeadCompleteInfo_BBBB alloc] initWithFrame:(CGRect) {
        .origin.x = 5,
    ...
    [tipView p_updateInTransaction:^(AAAA_SNLeadCompleteInfo_BBBB *tipView) {
        tipView.completeType = type;
        tipView.completion   = trueBlock;
        tipView.cancleCompletion  = callback;

        switch (type) {
            case AAAA_CompletingUserInfoType_BBBB_headicon:
                tipView.title = msg;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        {
            self.leftwardMarqueeView = [[AAAA_UUMarqueeView_BBBB alloc] initWithFrame:CGRectMake(54, 0, [UIScreen mainScreen].bounds.size.width- 54 - 30 - 10, 46) direction:AAAA_UUMarqueeViewDirection_BBBBLeftward];
...
- (NSUInteger)numberOfVisibleItemsForMarqueeView:(AAAA_UUMarqueeView_BBBB*)marqueeView {
    return 1;
}
...
- (CGFloat)itemViewHeightAtIndex:(NSUInteger)index forMarqueeView:(AAAA_UUMarqueeView_BBBB*)marqueeView {
...
- (CGFloat)itemViewWidthAtIndex:(NSUInteger)index forMarqueeView:(AAAA_UUMarqueeView_BBBB*)marqueeView {
    // for leftwardMarqueeView
    UILabel *content = [[UILabel alloc] init];
    content.font = [UIFont systemFontOfSize:13.0f];
    content.text = self.title;
