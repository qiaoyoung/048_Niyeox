//
// UIScrollView+AAAA_USERPullToRefresh_BBBB.m
//
...
static CGFloat const AAAA_USERPullToRefresh_BBBBViewHeight = 60;

@interface AAAA_USERPullToRefresh_BBBBArrow : UIView
...
@interface AAAA_USERPullToRefresh_BBBBView ()

@property (nonatomic, copy) void (^pullToRefreshActionHandler)(void);

@property (nonatomic, strong) AAAA_USERPullToRefresh_BBBBArrow *arrow;
@property (nonatomic, strong) UIActivityIndicatorView *activityIndicatorView;
@property (nonatomic, strong, readwrite) UILabel *titleLabel;
@property (nonatomic, strong, readwrite) UILabel *subtitleLabel;
@property (nonatomic, readwrite) AAAA_USERPullToRefresh_BBBBState state;
@property (nonatomic, readwrite) AAAA_USERPullToRefresh_BBBBPosition position;
...
#pragma mark - UIScrollView (AAAA_USERPullToRefresh_BBBB)
#import <objc/runtime.h>

static char UIScrollViewPullToRefreshView;

@implementation UIScrollView (AAAA_USERPullToRefresh_BBBB)
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(AAAA_USERPullToRefresh_BBBBPosition)position {
    ...
    if(!self.pullToRefreshView) {
        CGFloat yOrigin;
        switch (position) {
            case AAAA_USERPullToRefresh_BBBBPositionTop:
                yOrigin = -AAAA_USERPullToRefresh_BBBBViewHeight;
                break;
            case AAAA_USERPullToRefresh_BBBBPositionBottom:
                yOrigin = self.contentSize.height;
                break;
            default:
                return;
        }
        AAAA_USERPullToRefresh_BBBBView *view = [[AAAA_USERPullToRefresh_BBBBView alloc] initWithFrame:CGRectMake(0, yOrigin, self.bounds.size.width, AAAA_USERPullToRefresh_BBBBViewHeight)];
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler {
    [self addPullToRefreshWithActionHandler:actionHandler position:AAAA_USERPullToRefresh_BBBBPositionTop];
}

- (void)triggerPullToRefresh {
    self.pullToRefreshView.state = AAAA_USERPullToRefresh_BBBBStateTriggered;
    [self.pullToRefreshView startAnimating];
}

- (void)setPullToRefreshView:(AAAA_USERPullToRefresh_BBBBView *)pullToRefreshView {
...
- (AAAA_USERPullToRefresh_BBBBView *)pullToRefreshView {
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, AAAA_USERPullToRefresh_BBBBViewHeight);
        }
    }
}
...
#pragma mark - AAAA_USERPullToRefresh_BBBB
@implementation AAAA_USERPullToRefresh_BBBBView
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.state = AAAA_USERPullToRefresh_BBBBStateStopped;
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        switch (self.state) {
            case AAAA_USERPullToRefresh_BBBBStateAll:
            case AAAA_USERPullToRefresh_BBBBStateStopped:
                self.arrow.alpha = 1;
            ...
            case AAAA_USERPullToRefresh_BBBBStateTriggered:
                switch (self.position) {
                    case AAAA_USERPullToRefresh_BBBBPositionTop:
                        [self rotateArrow:(float)M_PI hide:NO];
                        break;
            ...
            case AAAA_USERPullToRefresh_BBBBStateLoading:
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        self.frame = CGRectMake(0, yOrigin, self.bounds.size.width, AAAA_USERPullToRefresh_BBBBViewHeight);
    }
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != AAAA_USERPullToRefresh_BBBBStateLoading) {
        ...
        if(!self.scrollView.isDragging && self.state == AAAA_USERPullToRefresh_BBBBStateTriggered)
            self.state = AAAA_USERPullToRefresh_BBBBStateLoading;
        else if(contentOffset.y < scrollOffsetThreshold && self.scrollView.isDragging && self.state == AAAA_USERPullToRefresh_BBBBStateStopped && self.position == AAAA_USERPullToRefresh_BBBBPositionTop)
            self.state = AAAA_USERPullToRefresh_BBBBStateTriggered;
        else if(contentOffset.y >= scrollOffsetThreshold && self.state != AAAA_USERPullToRefresh_BBBBStateStopped && self.position == AAAA_USERPullToRefresh_BBBBPositionTop)
            self.state = AAAA_USERPullToRefresh_BBBBStateStopped;
        else if(contentOffset.y > scrollOffsetThreshold && self.scrollView.isDragging && self.state == AAAA_USERPullToRefresh_BBBBStateStopped && self.position == AAAA_USERPullToRefresh_BBBBPositionBottom)
            self.state = AAAA_USERPullToRefresh_BBBBStateTriggered;
        else if(contentOffset.y <= scrollOffsetThreshold && self.state != AAAA_USERPullToRefresh_BBBBStateStopped && self.position == AAAA_USERPullToRefresh_BBBBPositionBottom)
            self.state = AAAA_USERPullToRefresh_BBBBStateStopped;
    } else {
        CGFloat offset;
...
- (AAAA_USERPullToRefresh_BBBBArrow *)arrow {
    if(!_arrow) {
        _arrow = [[AAAA_USERPullToRefresh_BBBBArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54, 22, 48)];
...
- (void)setTitle:(NSString *)title forState:(AAAA_USERPullToRefresh_BBBBState)state {
    ...
    if(state == AAAA_USERPullToRefresh_BBBBStateAll)
...
- (void)setState:(AAAA_USERPullToRefresh_BBBBState)newState {
    
    if(_state == newState)
        return;
    
    AAAA_USERPullToRefresh_BBBBState previousState = _state;
    _state = newState;
    ...
    switch (newState) {
        ...
        case AAAA_USERPullToRefresh_BBBBStateLoading:
            ...
            if(previousState == AAAA_USERPullToRefresh_BBBBStateTriggered && pullToRefreshActionHandler)
...
#pragma mark - AAAA_USERPullToRefresh_BBBBArrow

@implementation AAAA_USERPullToRefresh_BBBBArrow
