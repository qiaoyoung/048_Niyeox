...
@interface AAAA_ZMONMyQRcodeView_BBBB ()
...
@implementation AAAA_ZMONMyQRcodeView_BBBB
...
//    _box.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"chat_bg"]];
    ...
    UIImage *qrImage = [AAAA_LCQRCodeUtil_BBBB createQRimageString:[[NIMSDK sharedSDK].loginManager currentAccount] sizeWidth:220 fillColor:[UIColor blackColor]];
...
- (UIButton *)sureBtn {
    if (!_sureBtn) {
        ...
        [_sureBtn layoutButtonWithEdgeInsetsStyle:(AAAA_ImageTitleSpacing_BBBBEdgeInsetsStyleLeft) imageTitleSpace:10];
    ...
    [[PHPhotoLibrary sharedPhotoLibrary]performChanges:^{
          ...
          if (error) {
              dispatch_async(dispatch_get_main_queue(), ^{
                  ...
                  [AAAA_SVProgressHUD_BBBB showMessage:failed];
              });
          } else {
              NSString *success = LangKey(@"group_info_activity_update_success");//保存成功
              dispatch_async(dispatch_get_main_queue(), ^{
                  [AAAA_SVProgressHUD_BBBB showMessage:success];
              });
          }
      }];
}
