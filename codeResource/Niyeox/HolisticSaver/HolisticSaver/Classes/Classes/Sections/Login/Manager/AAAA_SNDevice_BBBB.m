...
@implementation AAAA_SNDevice_BBBB

+ (AAAA_SNDevice_BBBB *)sharedInstance {
    static AAAA_SNDevice_BBBB *_sharedDevice = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedDevice = [[AAAA_SNDevice_BBBB alloc] init];
    });
...
+ (NSString *)deviceIUUID {
    if (iuuid.length <= 0) {
        iuuid = [AAAA_SAMKeychain_BBBB passwordForService:SNUserDefault_Key_IUUID account:SNKey];
        if (iuuid.length <= 0) {
...
+ (void)updateDeviceIUUID {
    ...
    CFRelease(uuidRef);
    [AAAA_SAMKeychain_BBBB setPassword:iuuid forService:SNUserDefault_Key_IUUID account:SNKey];
}
