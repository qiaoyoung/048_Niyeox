//
//  AAAA_USERMultiRetweetAttachment_BBBB.m
//  NIM
//
...
@interface AAAA_USERMultiRetweetAttachment_BBBB ()
...
@implementation AAAA_USERMultiRetweetAttachment_BBBB
...
- (NSString *)encodeAttachment {
    ...
    NSDictionary *dict = @{CMType : @(AAAA_USERCustomAttachmentInfo_BBBBTypeMultiRetweet),
                           CMData : dataDic};
...
- (void)setAbstracts:(NSMutableArray<AAAA_USERMessageAbstract_BBBB *> *)abstracts {
    _abstracts = abstracts;
    NSMutableArray *abstractDics = [NSMutableArray array];
    for (AAAA_USERMessageAbstract_BBBB *obj in abstracts) {
    ...
    if (!messageAbstract) {
        ...
        for (id obj in messageAbstract) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                AAAA_USERMessageAbstract_BBBB *abstract = [AAAA_USERMessageAbstract_BBBB abstractWithDictionary:obj];
                if (abstract) {
...
- (NSString *)cellContent:(NIMMessage *)message {
    return @"AAAA_USERSessionMultiRetweetContentView_BBBB";
}
...
- (CGSize)contentSize:(NIMMessage *)message cellWidth:(CGFloat)width {
    ...
    for (AAAA_USERMessageAbstract_BBBB *abs in _abstracts) {
...
- (NSString *)filePath
{
    NSString *filePath = self.fileName ? [AAAA_USERFileLocationHelper_BBBB filepathForMergeForwardFile:self.fileName] : nil;
    return filePath;
}
...
#pragma mark - AAAA_USERMessageAbstract_BBBB
#define USER_Message_Abstract_Max_Len (32)

@implementation AAAA_USERMessageAbstract_BBBB
...
+ (instancetype)abstractWithMessage:(NIMMessage *)message {
    ...
    AAAA_USERMessageAbstract_BBBB *ret = [[AAAA_USERMessageAbstract_BBBB alloc] init];
    AAAA_FFFKitInfoFetchOption_BBBB *option = [[AAAA_FFFKitInfoFetchOption_BBBB alloc] init];
    option.session = message.session;
    option.message = message;
    AAAA_FFFKitInfo_BBBB *info = [[AAAA_MyUserKit_BBBB sharedKit].provider infoByUser:message.from option:option];
...
- (NSString *)abstract:(NIMMessage *)message {
    NSString *msg = [AAAA_USERMessageUtil_BBBB messageContent:message];
    NSMutableString *ret = [NSMutableString string];
    if (msg.length > USER_Message_Abstract_Max_Len) {
        NSArray *tokens = [[AAAA_FFFInputEmoticonParser_BBBB currentParser] tokens:msg];
        for (AAAA_NIMInputTextToken_BBBB *token in tokens) { //防止emoji表情被截断
